...
## Current Implementation: PayPal Web Payments
...
PAYPAL_RETURN_URL=http://localhost:5173/shop/success
PAYPAL_CANCEL_URL=http://localhost:5173/shop/cancel
```

#### 3. Test with Sandbox Accounts
PayPal provides test buyer accounts in the sandbox:
- Go to **Sandbox** → **Accounts**
- Use these test accounts to make purchases
- No real money is charged in sandbox mode

#### 4. Switch to Production
When ready for production:
1. Create a production app in PayPal Dashboard (**Live** tab)
...
PAYPAL_RETURN_URL=https://yourdomain.com/shop/success
PAYPAL_CANCEL_URL=https://yourdomain.com/shop/cancel
```
...
1. **User navigates to Imperial Shop**
   - Opens from navigation menu
   - Displays available bundles

2. **User clicks "Buy Now"**
   - Frontend calls `POST /v1/payments/create`
   - Backend creates PayPal order
   - Transaction saved to database with "pending" status

3. **User redirected to PayPal**
   - Logs in to PayPal account (or uses card)
   - Reviews and approves payment

4. **User returns to application**
   - URL contains PayPal order token
   - Frontend calls `POST /v1/payments/capture`
   - Backend captures payment from PayPal
   - Updates transaction status to "completed"
   - Credits imperials to user's account

5. **Success message displayed**
   - User sees confirmation
   - Imperial balance updated
...
1. **Webhook Verification**: TODO - Implement PayPal webhook signature verification
2. **Idempotency**: Transactions are keyed by `providerOrderId` to prevent double-crediting
3. **Authentication**: All payment endpoints require JWT authentication
4. **HTTPS**: Production must use HTTPS for all payment flows
5. **PCI Compliance**: PayPal handles all card data - we never see it
...
### 1. **Web Browser** (Direct Web Payments) ✅ IMPLEMENTED

**Options:**
- **PayPal** (Recommended) - Implemented
- **Stripe** - Can be added similarly
- **Paddle** - Merchant of record (handles VAT/tax automatically)

**Implementation:**
```typescript
// Backend: apps/api/src/modules/payments/routes.ts
POST /v1/payments/create
POST /v1/payments/capture
...
- Need PCI compliance (mitigated by using PayPal/Stripe
